...
def map_host_to_dest_to_nutsresult(
    ...
    """
    Maps a host's name to its corresponding destination
    and calls a helper function to further map
...
def map_dest_to_nutsresult(
    ...
    the destination information is missing in the result,
    and we do not know which destination actually failed.
    It is therefore necessary to patch the destination onto the Result object
    to later know which host-destination pair actually failed.
    ...
    Tasks per host (such as pinging a series of destinations) are nornir (sub)tasks
    that are executed from within another task.
    For the main task to finish properly, we must return a single Result, such as
    `Result(host=task.host, result="All pings executed")`, that confirms
    the fulfillment of the subtasks.
    All subsequent Result objects are then the completed subtasks.
    We do not need the first Result that simply confirms the fulfilled subtasks,
    therefore we skip it explicitly and return `task_results[1:]`.
    ...
    """
    return {
        single_result.destination: nuts_result_wrapper(  # type: ignore[attr-defined]
            single_result, single_transform
        )
...
def nuts_result_wrapper(
    ...
    :param single_transform: Function that accepts the nornir_result
    ...
    if nornir_result.failed:
        return NutsResult(
            failed=True, exception=nornir_result.exception  # type: ignore[attr-defined]
        )
    try:
